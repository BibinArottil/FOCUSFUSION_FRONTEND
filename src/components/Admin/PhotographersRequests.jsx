...
function PhotographersRequests() {
  ...
  const handleReject = async (id,email) => {
    try {
      await axios.post("/admin/reject", { id,email })
      .then((res)=>{
        if (res.data.success) fetchData();
  ...
  const viewImage=async(id)=>{
    try {
      await axios.post('/admin/view-images',{
        id
      }).then((res)=>{
  ...
  const columns = [
    ...
    {
      name: "Images",
      selector: (row) =><AiFillFolderOpen onClick={()=>viewImage(row.id)} className="text-xl cursor-pointer"/> ,
    },
    {
      name: null,
      cell: (row) => (
  ...
  return(
    <div className="h-full w-full">
      <div className="flex flex-col px-20 py-5">
        ...
        <Link to="/admin/photographers/list">
            Photographers
        </Link>
          </button>
      </div>
